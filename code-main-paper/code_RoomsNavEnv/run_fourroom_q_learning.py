    ...
    for i in range(1, n_averaged+1):
        ...
        for key in sorted_keys_by_length:
            if "R_original" in key or\
                ...
                "R_reward_design_env=room_n_states=50_len_active_set_chosen=4" in key or\
                "R_potential_shaping" in key or "Pot_craft" in key or\
                "R_landmark_handpicked_env=room_n_states=50_len_active_set_chosen=6" in key or \
                "R_landmark_state_design_env=room_n_states=50_len_active_set_chosen=6" in key or \
                "Reward_all_state_design" in key: # execute only for the |Z|=5
                print("\n================")
                print(key)
                print("================\n")
...
if __name__ == "__main__":
    ...
    dir_path ="results/designed_rewards_room_const_middle_sub_optimal_IR"

    tol = 1e-10

    dict_file = input_from_file(dir_path, filename)
    # print(dict_file["H_set"])
    # print(dict_file["R_reward_design_env=room_n_states=50_len_active_set_chosen=6"].reshape(50, 4))
    # exit(0)
    ...
    get_convergence_numbers_q_learning(dict_file, env_orig, pi_target_d, pi_target_s,
                                       ...
                                       outfile_name="Q_convergence_room_{}_const_middle__sub_optimal_IR".format(is_delta_s_const),
