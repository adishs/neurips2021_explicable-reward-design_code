    ...
    for i in range(1, n_averaged+1):
        ...
        for key in sorted_keys_by_length:
            if "R_original" in key: # execute only for the |Z|=5
                ...
                print(key)
                print("================\n")
                dict_tmp = q_learning_step.q_learning(
                    env=env_0_1, max_episode=max_episode, max_step=max_step, epsilon=epsilon, alpha=alpha,
                    env_orig=env_0_1,
                    pi_t=pi_target_d, name=key, usereward_env=False,  tol=tol
                )
            ...
            if "R_reward_design_env=chain_n_states=41_len_active_set_chosen=7" in key: # execute only for the |Z|=5
                ...
                env_orig_abstracted_reward = mapping_rewards_rom_abstraction_to_orig_env(env_0_1,
                                                                                         env_R_input)
                dict_tmp = q_learning_step.q_learning(
                    env=env_orig_abstracted_reward, max_episode=max_episode, max_step=max_step, epsilon=epsilon, alpha=alpha,
                    env_orig=env_0_1,
                    pi_t=pi_target_d, name=key, usereward_env=True,  tol=tol
                )
                dict_accumulator_main = append_to_accumulator(dict_accumulator_main, dict_tmp)
            ...
            if "R_potential_shaping_cont" in key:
                ...
                #                                                                          env_R_input)
                dict_tmp = q_learning_step.q_learning(
                    env=env_orig_abstracted_reward, max_episode=max_episode, max_step=max_step, epsilon=epsilon,
                    alpha=alpha,
                    env_orig=env_0_1,
                    pi_t=pi_target_d, name=key, usereward_env=True, tol=tol
                )
...
if __name__ == "__main__":
    ...
    env_0_1 = linekey_env.discreteEnv(env_args)

    env_net = linekey_abstration.discreteEnv(env_0_1, epsilon_net=epsilon_net,
    ...
    dir_path ="results/designed_rewards_chain_key_const_middle_sub_optimal_IR"
