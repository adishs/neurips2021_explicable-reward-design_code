...
from linekey_reward_design_exprd import *
...
def get_landmark_reward_for_chain(env_orig, pi_d, n_landmarks, R_scalar_for_landmark,
    ...
    if n_landmarks == 1:
        ...
        states_for_landmark = np.array([0, 7, 4, 10], dtype=int) # n_states = 15
...
def get_important_states_by_shaping(env_orig, pi_d, pi_s, R_max, H_set, s_active=None,
    ...
    for i in range(1, n_nonzero_states+1):
        min_value = np.inf
        state_chosen = None
        R_shaped = None
...
def calculate_PBRS_orig(env_orig, env_abstr, n_times_state_piced=10,
    ...
    T, R = linekey_abstration.get_sampling_T_ground(env_orig, n_times_state_piced, n_times_action_picked)
    reward_PBRS = {}
...
if __name__ == "__main__":
    ...
    env = linekey_env.discreteEnv(env_args)
    env_orig = linekey_abstration.discreteEnv(env, epsilon_net=epsilon_net, num_traj=0, len_traj=500)
    for key in env_args:
    ...
    _, PBRS_cont = calculate_PBRS_orig(env, env_orig, n_times_state_piced=20,
                        n_times_action_picked=10)

    accumulator_for_file["R_potential_shaping_cont"] = PBRS_cont.flatten()

    H_set = [1, 4, 8, 16, 32]
    #get_H_set(env_orig) # (1/1-gamma)//2, ...., 1/(1-gamma)
    ...
    # ===========================================
    # print(delta_s_array_orig)
    # # print(np.minimum(delta_s_array_orig, upper_c_for_delt_s))
    # print(upper_c_for_delt_s)
    # exit(0)
    ...
    states_active, R_design = get_important_states_by_shaping(env_orig, pi_d=pi_target_d, pi_s=pi_target_s, R_max=R_max, H_set=H_set, s_active=s_active,
                                                ...
                                                dict_s_opt_actions_arr=dict_s_opt_actions_arr,
                                                state_only_reward_flag=state_only_reward_flag, is_delta_s_const=is_delta_s_const, upper_c_for_delt_s=upper_c_for_delt_s, tol=tol,
                                                env_name="chain", exp_name="reward_design", exp_iter=1, out_file_name="designed_rewards")
    ...
    write_into_file(accumulator_for_file, exp_iter=1, out_folder_name="designed_rewards_chain_key_const_middle_sub_optimal_IR",
