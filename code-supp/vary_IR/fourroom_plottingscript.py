...
def input_dict_from_file_and_plot_steps_and_exp_reward(dict_file_from_paper, dict_file_q, n_file=10,
                                                       out_folder_name="out_folder", each_number=50, t=100, fig_name=""):
    ...
    for key in keys:
        ...
        if key == "R_landmark_state_design_env=room_n_states=50_len_active_set_chosen=6_expected_reward":
            ...
            plt.errorbar(range(0, up_to_power_2 + 2),
                         dict_file_from_paper[key][array_index_to_plot],
                         label=r"$\textsc{Orig}$",
                         color=color_codes[0], marker="s", ls="-.", lw=4, markersize=10,
                         yerr=dict_file_from_paper["SE_Reward_orig"][array_index_to_plot])
        elif key == "R_potential_shaping_expected_reward":
            plt.errorbar(range(0, up_to_power_2 + 2),
                         dict_file_from_paper[key][array_index_to_plot],
                         label=r"$\textsc{PBRS}$",
                         color='#0a81ab', marker=".", ls="-.", markersize=18, lw=4,
                         yerr=dict_file_from_paper["SE_Reward_pot"][array_index_to_plot])
    ...
    outFname = os.getcwd() + "/" + out_folder_name + "/{}.pdf".format(fig_name)
        ...
        with open(file_path + expname + str(file) + '.txt') as f:
            ...
            for line in f:
                ...
                if read_line[0] != '#':
                    ...
                    if read_line[0] == "R_reward_design_env=room_n_states=50_len_active_set_chosen=6_expected_reward":
...
if __name__ == "__main__":
    ...
    input_dir_path_q_learning_convergence_numbers = \
        "results/Q_learning_data/{}/".format(input_folder)
    ...
    dict_file_from_paper = input_from_file(
    ...
    input_dict_from_file_and_plot_steps_and_exp_reward(dict_file_from_paper, dict_file_from_paper,
                                                       n_file=n_averaged, out_folder_name=out_folder_name,
                                                       each_number=each_number, t=t, fig_name=figure)
