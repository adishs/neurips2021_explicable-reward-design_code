...
def q_learning(env, max_episode=3000, max_step=500, epsilon=0.1, alpha=0.5, env_orig=None,
    ...
    for i_episode in range(max_episode + 1):
        ...
        state = np.random.choice(np.arange(env.n_states), p=env_orig.InitD)

        pi_d = make_greedy_det_policy(Q)

        V_pi_d = MDPSolver.computeValueFunction_given_policy_linear_program(env_orig, pi_d, env_orig.reward)

        exp_reward = np.dot(env.InitD, V_pi_d)
        result_dict[name + "_expected_reward"].append(exp_reward)

        visited_goal_step = copy.deepcopy(max_step)
        visit_goal_flag = False
        for step in range(max_step+1):
            ...
            if state == env.n_states -1 and env.terminal_state ==1: #if we visit  a terminal state break
                break
