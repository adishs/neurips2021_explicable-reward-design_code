...
class discreteEnv:
    def __init__(self, env_args):
        ...
        self.done = False
        self.reward = None
        self.input_dim_to_reinforce = len(self.states_in_zero_one_interval_with_key)
    ...
    def transitions_LEFT(self):
        ...
        if self.current_state_x[0] + move_size_x <= 0:
            self.current_state_x = (0, self.key)
        else:
            next_state = (self.current_state_x[0] + move_size_x, self.key)
            self.current_state_x = next_state
    ...
    def transitions_RIGHT(self):
        ...
        if self.current_state_x[0] + move_size_x >= 1:
            self.current_state_x = (1, self.key)
        else:
            next_state = (self.current_state_x[0] + move_size_x, self.key)
            self.current_state_x = next_state
    ...
    def pick_KEY(self):
        ...
        if self.key_location[0][0] <= self.current_state_x[0] <= self.key_location[1][0]:
            self.key = 1
            self.current_state_x = (self.current_state_x[0], 1)
        return self.current_state_x
    #enddef
    ...
    def state_to_one_hot_encoding(self, state):
        reinforce_state_representation = np.zeros(len(self.states_in_zero_one_interval_with_key))
        # reinforce_state = (state[0], 0)  # state without key
        index_of_state = self.mapping_from_float_coords_to_int_coords[tuple(state)]
        reinforce_state_representation[index_of_state] = 1  # state is on
        return reinforce_state_representation
    # enddef
...
if __name__ == "__main__":
    ...
    # print(env.states_in_zero_one_interval_with_key[22])
    # print(env.states_in_zero_one_interval_with_key[25])
    # exit(0)

    print(tuple(env.reset()))
    exit(0)

    for s in range(50000):
        action = np.random.choice(np.array([0, 1, 2], dtype=int))
        curr_x = env.current_state_x
        curr_state = np.array([curr_x])
        # action = agent.get_action(curr_state)[0]

        next_state, reward, done, _ = env.step(action)
        print("s={}, a={}, s_n={}, r={}".format(curr_state,
    ...
    s_active = [19, 18, 0, 10, 5, 15]
    # s_active = [19, 18, 0, 1]
    # s_active = range(0, 20)

    _, R_sol = reward_design_model_based(env_disc, pi_d=pi_d_disc, pi_s=pi_s_disc, R_max=100,
    ...
    agent = reinforce.Agent(env_cont, discrete_env=discrete_env_shaped, usereward_env=True)
    result_dict = agent.train(20000, 500, name="discrete")
